  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        onClick={() => setOpen(v => !v)}
        className={cn(
          "w-full flex items-center gap-3 bg-white/5 border rounded-xl px-4 py-3 text-sm text-left transition-all",
            ...
            <div className="overflow-y-auto max-h-52">
              ...
              {filtered.length === 0 && (
                <div className="px-4 py-4 text-sm text-white/30 text-center">
                  No match — choose <button className="text-violet-400 font-medium" onClick={() => { setManualMode(true); setOpen(false); }}>type manually</button>
                </div>
              )}
            </div>
          </motion.div>
...
function Field({
  label, placeholder, value, onChange, type = "text", icon: Icon, required, hint, children,
}: {
  label: string; placeholder?: string; value?: string;
  onChange?: (v: string) => void; type?: string;
  icon?: any; required?: boolean; hint?: string; children?: React.ReactNode;
}) {
  return (
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          ...
          <input
            type="tel" placeholder="e.g. 08012345678" value={phone}
        ...
        >
          ...
          {step === 2 && (
            <>
              ...
              <Field label="Intended Course" placeholder="e.g. Law, Medicine, Engineering" value={course} onChange={setCourse} icon={BookOpen} required />
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-white/40 tracking-widest uppercase">
          ...
          {step === 3 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-[11px] font-bold text-white/40 tracking-widest uppercase">JUPEB Subjects <span className="text-rose-400">*</span></label>
                <span className={cn(
                  "text-[11px] font-bold px-2.5 py-0.5 rounded-lg border transition-all",
              ...
              {subjects.length === 3 && (
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[11px] text-emerald-400/70 flex items-center gap-1.5">
      ...
      <div className="w-full max-w-md relative">
        ...
        >
          ...
          <div className="relative bg-gradient-to-br from-[#1a1030] via-[#151025] to-[#0e0e1a] px-6 pt-8 pb-6 border-b border-white/6 overflow-hidden">
            ...
            <div className="absolute -bottom-6 -left-6 w-32 h-32 rounded-full bg-emerald-500/6 blur-2xl" />
